...
export default function QRPage() {
	...
	return (
        <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            <aside
                className={`hidden lg:flex flex-col bg-white border-r border-slate-200 h-screen sticky top-0 transition-all duration-300 ${isShowNavigation ? "w-64" : "w-20"} overflow-hidden`}
            >
                ...
                <nav className="flex-1 px-4 space-y-2">
                    ...
                    <Link href="/links">
                        <NavItem icon={<LinkIcon size={20} />} label="My Links" isOpen={isShowNavigation} active={false} />
                    </Link>
                    <Link href="/qr-codes">
                        <NavItem icon={<QrCode size={20} />} label="QR Codes" isOpen={isShowNavigation} active={true} />
                    </Link>
                </nav>
            </aside>
                    ...
                    <div className="max-w-5xl mx-auto">
                        <div className="mb-8">
                            ...
                            <p className="text-slate-500 text-sm">
                                You have {filteredLinks.length} active QR Codes
                            </p>
                        </div>
                        ...
                        <div className="grid gap-4">
                            {isLoading ? (
                                [1, 2, 3].map((i) => (
                                    <div key={i} className="h-32 bg-white animate-pulse rounded-2xl border border-slate-200" />
                                ))
                            ) : filteredLinks.length > 0 ? (
                                filteredLinks.map((link: any) => (
                                    <div
                                        key={link.id}
                                        className="group bg-white p-4 rounded-2xl border border-slate-200 hover:shadow-md transition-all flex flex-col md:flex-row items-center gap-6"
                                    >
                                        <div className="w-24 h-24 bg-white rounded-xl border border-slate-100 flex items-center justify-center shrink-0 p-2 shadow-sm">
                                            <QRCodeSVG
                                        ...
                                        <div className="flex-1 min-w-0 text-center md:text-left">
                                            <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                                                <h3 className="font-bold text-slate-900 truncate text-lg group-hover:text-[#6C5CE7] transition-colors">
                                                    {link.short_code}
                                                </h3>
                                                <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-bold uppercase shrink-0">
                                                    {link.click_count || 0} Clicks
                                                </span>
                                            </div>
                                            <p className="text-sm text-slate-400 truncate mb-2 max-w-md">
                                                {link.original_url}
                                            </p>
                                        ...
                                        <div className="flex items-center gap-2 border-t md:border-0 pt-4 md:pt-0 w-full md:w-auto justify-center shrink-0">
                                            ...
                                            <button
                                                ...
                                                className="p-2.5 hover:bg-slate-100 rounded-xl text-slate-500"
                                            >
                                                <MoreHorizontal size={20} />
                                            </button>
                                            <button
                                                onClick={() => handleDelete(link.short_code)}
                                                className="p-2.5 hover:bg-red-50 rounded-xl text-red-500"
                                            >
                                                <Trash2 size={20} />
                                            </button>
                                            <a
                                                ...
                                                className="p-2.5 bg-[#6C5CE7] text-white rounded-xl hover:bg-[#5b4cc4] shadow-sm transition-all"
                                            >
                                ...
                                <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                                    <p className="text-slate-400 mb-4">No QR codes found.</p>
                                    <Link href='/home' className="px-6 py-2 bg-[#6C5CE7] text-white rounded-xl font-bold inline-block">
                                        Create a QR
                                    </Link>
                                </div>
                            )}
            ...
            {isModalEditOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in-95 duration-200">
                        {/* Preview Section */}
                        <div className="md:w-1/3 bg-slate-50 p-6 border-r border-slate-100 flex flex-col items-center justify-center">
                            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Preview</h4>
                        ...
                        <div className="md:w-2/3 p-6 lg:p-8 relative">
                            <button
                            ...
                            <div className="mb-6">
                                <h3 className="text-xl font-black text-slate-900">Edit QR Code</h3>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-700 mb-1">Custom Alias</label>
                                    <input
                                        type="text"
